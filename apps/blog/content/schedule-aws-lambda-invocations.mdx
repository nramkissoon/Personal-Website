...
description: "Guide on how use AWS CDK and EventBridge to invoke a Lambda function on a set schedule."
canonical: "https://medium.com/geekculture/schedule-aws-lambda-invocations-with-eventbridge-and-aws-cdk-fbd7e4e670bb"
date: "2021-09-13"
---
...
The above commands will initialize a new CDK project in the `PROJECT_NAME` directory and add a few
...
```typescript filename=lambda-construct.ts
...
The `SimpleLambdaConstruct` class exposes the Lambda function to the outside world in the class variable `this.lambda`.

## Creating an AWS EventBridge Rule and Integrating Lambda

Now, let's implement our Event Rule Construct in `event-rule.ts`.

```typescript filename=event-rule.ts
...
```typescript filename=lambda-schedule-stack.ts
import * as cdk from "@aws-cdk/core";
