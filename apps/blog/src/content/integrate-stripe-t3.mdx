...
```ts,src/server/stripe/client.ts
...
```ts,src/pages/api/stripe-webhook.ts
...
```ts,src/server/stripe/stripe-webhook-handlers.ts
...
```ts,src/server/trpc/router/stripe.ts
import { env } from "../../../env/server.mjs";
...
```ts,src/server/trpc/router/user.ts
import { router, protectedProcedure } from "../trpc";
